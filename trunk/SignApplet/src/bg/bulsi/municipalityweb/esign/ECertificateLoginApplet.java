...
public class ECertificateLoginApplet extends SmartCardApplet {
	...
	protected void signButtonPressed(X509Certificate certificate, JSObject browserWindow,
		...
		JSObject mainForm = (JSObject) browserWindow.eval(new StringBuilder().append(
				"document.getElementById('").append(formId).append("')").toString());
		...
		try {
			...
			String signedMessage = CertificateSignUtils.signXML(message.toString(),
			...
			mainForm.eval(new StringBuilder().append("document.getElementById('").append(
					loginButtonId).append("').onclick();").toString());
